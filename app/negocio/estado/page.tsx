...
export default function EstadoPage() {
  ...
  const [consultaFactura, setConsultaFactura] = useState("");
  const [tramite, setTramite] = useState<Tramite | null>(null);
  const [loading, setLoading] = useState(false);
  const [realizadoBusqueda, setRealizadoBusqueda] = useState(false);
  const [errorConsulta, setErrorConsulta] = useState("");

  // Polling para sincronizar en tiempo real con las acciones del inspector para el RUC o Factura buscado
  useEffect(() => {
    if (!consultaRuc && !consultaFactura) return;

    let active = true;

    async function cargarTramite() {
      try {
        const queryParam = consultaRuc ? `ruc=${consultaRuc}` : `factura=${consultaFactura}`;
        const res = await fetch(`/api/tramites/mi-tramite?${queryParam}&t=${Date.now()}`, {
          cache: "no-store",
        ...
        if (res.ok && active) {
          ...
          if (!data) {
            setErrorConsulta(
              consultaRuc
                ? "No se encontró ningún trámite registrado para el RUC ingresado."
                : "No se encontró ningún trámite registrado para la Factura de pago ingresada."
            );
          } else {
            setErrorConsulta("");
          }
  ...
  const handleBuscar = (e: React.FormEvent) => {
    ...
    // Si son 11 dígitos y empieza con 20 es un RUC. Si no, se considera número de factura
    if (/^\d+$/.test(val) && val.length === 11) {
      ...
      if (val.length < 4) {
        setErrorConsulta("Ingresa un código de factura o RUC válido.");
        setRealizadoBusqueda(false);
        return;
      }
      setConsultaFactura(val);
      setConsultaRuc("");
    }
  };
    ...
    return (
      <div className="mx-auto max-w-2xl mt-10">
        <div className="text-center mb-8">
          <PageHeading
            title="Mi trámite"
            description="Consulta el avance de tu solicitud y las acciones que requiere ingresando tu RUC o Factura de Pago."
          />
        </div>

        <form onSubmit={handleBuscar} className="rounded-2xl border border-[var(--border)] bg-white p-6 shadow-md">
          <label className="text-sm font-semibold text-slate-700 block mb-3">
            Ingresa el RUC de la Empresa o Número de Factura:
          </label>
          <div className="flex gap-2">
            <div className="relative flex-1">
              <input
                ...
                placeholder="20123456789 o F001-000123"
                maxLength={30}
                value={searchInput}
  ...
  if (loading) {
    return (
      <div className="flex min-h-[400px] flex-col items-center justify-center gap-3 text-[var(--muted)]">
        ...
        <p className="text-sm font-semibold">Buscando trámite para {consultaRuc || consultaFactura}...</p>
      </div>
    );
  }
      ...
      <div className="mx-auto max-w-2xl mt-10">
        <div className="rounded-2xl border border-dashed border-slate-300 bg-white p-10 text-center shadow-sm">
          ...
          <p className="mt-2 text-sm text-[var(--muted)] leading-relaxed">
            No se encontró ninguna solicitud de Licencia de Funcionamiento activa para <strong className="font-mono">{consultaRuc || consultaFactura}</strong>.
          </p>
