...
const datosDesarrollo: Record<string, DatosRuc> = {
  ...
  // RUC real de Centro Comercial Plaza Norte para pruebas de ubicación
  "20514020907": {
    ruc: "20514020907",
    razonSocial: "CENTRO COMERCIAL PLAZA NORTE S.A.C.",
    domicilioFiscal: "AV. SIETE NRO. 229 URB. RINCONADA BAJA (A 3 CUADRAS DE MOLICENTRO) LIMA - LIMA - LA MOLINA",
    estado: "ACTIVO",
    condicion: "HABIDO",
    distrito: "LA MOLINA",
    provincia: "LIMA",
  ...
  // RUC inactivo para pruebas
  "20999999991": {
    ruc: "20999999991",
    razonSocial: "NEGOCIO INACTIVO SAC",
    domicilioFiscal: "AV. LARCO NRO. 789 URB. SAN ANDRES",
    estado: "INACTIVO",
    condicion: "HABIDO",
    distrito: "TRUJILLO",
    provincia: "TRUJILLO",
  ...
  // RUC de otro distrito (LIMA) para pruebas
  "20999999992": {
    ruc: "20999999992",
    razonSocial: "NEGOCIO FUERA DE TRUJILLO SAC",
    domicilioFiscal: "AV. JAVIER PRADO ESTE NRO. 1024",
    estado: "ACTIVO",
    condicion: "HABIDO",
    distrito: "LIMA",
    provincia: "LIMA",
...
// Consulta con la API pública gratuita de apis.net.pe (sin token, con límite de tasa)
...
export async function consultarRuc(ruc: string): Promise<DatosRuc> {
  ...
  if (!token) {
    // 1. Intentar con API pública gratuita (funciona sin token)
    const datosPublico = await consultarRucPublico(ruc);
    if (datosPublico) return datosPublico;

    // 2. Fallback a datos hardcodeados o autogenerados en desarrollo
    if (process.env.NODE_ENV === "development") {
      const datoLocal = datosDesarrollo[ruc];
      if (datoLocal) return datoLocal;

      // Autogenerar dinámicamente datos de RUC válidos para Trujillo si no está registrado
      // Autogenerar dinámicamente datos de RUC realistas y diferentes basados en los dígitos del RUC ingresado
    ...
    throw new ConsultaRucError(
      "No se pudo consultar el RUC. Verifica tu conexión o agrega SUNAT_RUC_API_TOKEN en .env.local.",
      503,
    );
  }

  // Si hay token, usar la API configurada. Por defecto asumimos la API de apis.net.pe v2 que ofrece tokens gratuitos
  ...
  if (isApisNetPe) {
    // Parser para la API de apis.net.pe (camelCase)
    const parsed = apisNetPeSchema.safeParse(await response.json());
    if (!parsed.success) {
    ...
    // Parser para la API de Decolecta (snake_case)
